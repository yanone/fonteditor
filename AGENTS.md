...
`previewrelease.sh` pushes unpushed `main` commits and starts the Preview Release workflow. That workflow waits for a green `ci.yml` run on that commit, builds with `DATE-build-N`, then creates the `v0.0.0-preview.DATE.N` tag and prerelease and deploys to preview.editor.counterpunch.space. It does not rewrite `CHANGELOG.md` or re-run the test suite.
...
- `developer-docs/LOCALSTORAGE_SETTINGS.md` - localStorage settings key reference (auto-generated by `node generate-localstorage-docs.mjs`)
- `developer-docs/SESSIONSTORAGE_SETTINGS.md` - sessionStorage settings key reference (auto-generated by `node generate-sessionstorage-docs.mjs`)
...
When a `localStorage` key is added, removed, or changed, `developer-docs/LOCALSTORAGE_SETTINGS.md` MUST be regenerated in the same change using `node generate-localstorage-docs.mjs` (from repo root). Do not edit that file by hand. That command is also included in `npm run generate-docs`.

When a `sessionStorage` key is added, removed, or changed, `developer-docs/SESSIONSTORAGE_SETTINGS.md` MUST be regenerated in the same change using `node generate-sessionstorage-docs.mjs` (from repo root). Do not edit that file by hand. That command is also included in `npm run generate-docs`.
...
This project is indexed by GitNexus as **editor** (25969 symbols, 51328 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
