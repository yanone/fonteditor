...
Idle committed packets (local, remote, undo, redo) capture a lock on **this** window's canvas, then re-apply it after model sync and after the editing-font reshape. They do not replay another window's gesture lock. Edit mode keeps the active glyph origin screen-stationary: LTR uses the glyph-space lower-left `(0, 0)`, RTL uses the lower-right `(xAdvance, 0)`. The exception is a sidebearing edit that includes this window's active glyph (or a font-scoped sidebearing packet with no glyph names): that packet keeps the active glyph layer's bounding-box center screen-stationary. Text-mode kerning value/group packets keep the pair's reference glyph screen-stationary so the caret can move with the overlay; other text-mode packets keep the caret screen-stationary. Live drags and live sidebearing bursts on that window still bypass this lock. Keyboard zoom and collapse-resize still lock the edit-mode bbox center independently of this idle funnel.
