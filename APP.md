...
When a text-mode kerning value edit reshapes the text run (kerning-only compile, matching undo/redo, and the deferred full compile after that edit), keep the text cursor screen-stationary by adjusting `panX`: capture the cursor screen position immediately before reshape and re-apply after. For RTL pairs with a negative kerning value, draw the cursor at the left edge of the kerning distance so that junction matches LTR semantics; positive RTL and all LTR pairs keep the default between-glyph cursor edge. While waiting for that reshape, the active kerning overlay band and bottom marker must also grow and shrink from the current caret X (not from the opposite glyph edge), so live value nudges match LTR directionality.
