...
When a text-mode kerning value edit reshapes the text run (kerning-only compile, matching undo/redo, and the deferred full compile after that edit), keep the kerning pan anchor screen-stationary by adjusting `panX`: for LTR, capture the glyph left of the pair (`firstCluster.x`) immediately before reshape and re-apply after so the caret is free to move on screen with the kerning; for RTL, keep the caret screen-stationary until RTL anchoring is revisited. For LTR, the caret stays at the between-glyph edge (`second.x`): the left side of the kerning overlay for negative values and the right side for positive values, so it moves with the kerning as the first glyph's advance changes. For RTL negative pairs, draw the cursor at the left edge of the kerning distance. While waiting for that reshape, the active kerning overlay band and bottom marker must also grow and shrink from the current caret X (not from the opposite glyph edge). Live advance preview: LTR adjusts the first glyph's advance and pans from the left glyph of the pair (not the caret); RTL shifts the first (right) glyph via `dx` so the second (left) glyph and caret stay put until compile reshape.
