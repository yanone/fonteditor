...
The Editing View title bar exposes sticky tools left of the View menu so mouse-only users can reach the same outline actions that Cmd/Alt unlock. In text mode the Text tool (T) is always highlighted active and the other tools are unavailable. In edit mode the sticky tools are Select (V), Pen (P), Insert node (I), and Convert (C); T exits edit mode through the same path as Escape. Pen draws new paths; Insert inserts a node on a segment; Convert turns a straight segment into a curve; Select is the default object-selection tool. Select and Pen are always available in edit mode; Insert when path segments exist; Convert when straight line segments exist. Entering a glyph for editing refreshes Insert and Convert through the same availability sync used after structural deletes and undo/redo, as soon as that glyph’s outlines are loaded. Entering edit mode always defaults to Select, even when the layer has nothing to select.
