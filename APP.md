...
Clicking a path segment (the middle of the segment, not the end-zone that counts as a node hover) selects that segment’s two on-curve nodes. Double-clicking a segment selects every node on that contour. Shift-clicking a segment toggles it in the current selection: if a neighboring segment is already selected, only the unshared end node is added or removed; if the segment sits between two selected neighbors, both of its nodes are added or removed while each neighbor’s far node stays selected. `cmd`/`ctrl` does not add to or remove from object selection (it cuts or inserts).
