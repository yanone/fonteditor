...
Sidebearing drags skip `saveLayerData()` during the drag, mirroring the anchor drag pattern. Each handle-delta tick syncs the source layer and runs **one** `computeLayerRecompositionClosure({ scope: 'visible' })` (transitive retain expansion included), refreshes advances only for `source ∪ recomposeTargets` (not invalidate-only manuals), and caches `allTargets` for preview. The live funnel stages the latest coalesced targets via `stageLiveDragPreviewFromModel` with compile-facing (`toCompileJSON`) serialization and must not re-run the closure. After a successful nonempty stage it requests compile. Preview results use a latest-applied lane: a completed stage may paint while a newer stage is queued, and only a result older than the current painted preview is rejected. This prevents revision supersession from delaying visible recomposition until pointer idle. Sidebearing preview font swaps reshape the text run. On mouseup, the editor drains the live funnel, runs **one** `scope: 'all'` closure in `refreshFinalSidebearingWorkerStateBeforeCommit` (stashed for serialize-only sync — no second full closure), applies delta-only advance pan vs the last live advances, and syncs `source ∪ recomposeTargets` through the authoritative Yjs packet (logical `toJSON`). Yjs worker mirrors remain logical. Compile-facing automatic data comes from (1) the preview overlay during live drag and (2) Rust compile-read bake of logical `=+/-=` layers when building the font handed to fontc. Each preview stage replaces its physical overlay atomically; overlay layers are skipped by Rust bake. The final `saveLayerData('mouse-drag-sidebearing')` fires on mouseup for the Yjs/collaboration sync and undo history.
