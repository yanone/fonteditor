...
19. **Replay-target refresh keeps app-scheduled compiled fonts congruent.** After `awaitCommittedWorkerCacheSettled` resolves, the authoritative binary Yjs update has reached Rust through `forwardWorkerYjsUpdate`. That forwarded Yjs update is necessary, but it is not sufficient as the only pre-compile freshness proof for local sender commits: app-driven linked-window fuzzing caught committed local anchor edits and undo/redo packets where the JS model had changed but the cached editing font was compiled from stale worker layer data.

Therefore `handleCommittedChangeRefresh` MUST call `refreshWorkerCacheForReplayTargets` for every local committed packet that carries explicit `workerReplayTargets`, not only for undo/redo or receiver-side packets. This extra refresh serializes the exact affected layers from the current bridge Y.Doc and pushes them as an incremental Yjs update to Rust before the app-scheduled compile; the JS object model and stored babelfont JSON are downstream views and MUST NOT be preferred over the bridge as the refresh source. Because that refresh is itself a worker-document/cache mutation, the committed funnel MUST wait for `awaitCommittedWorkerCacheSettled` again after the refresh and before requesting the editing compile. This replaces the older local-sender assumption that forwarded Yjs worker updates alone always left layer caches compile-ready.

The rule preserves the no-full-document constraint: steady-state local edits, undo, redo, linked-window replay, cloud replay, Python execution, and feature-code commits still MUST NOT use `storeFontJson`, `seedYdoc`, `initYdoc`, full Yjs-state resend, or full babelfont JSON repair as an edit-time correctness path. The receiver (remote) branch still runs the scoped cache refresh because linked windows need that pre-compile refresh on their own worker/cache side, and it must obey the same post-refresh worker-settled barrier.

    See `webapp/js/change-bridge-init.ts` `handleCommittedChangeRefresh` and `awaitCommittedWorkerCacheSettled`.
