...
1. Interactive outline edits must compile live during the interaction.
2. Interactive anchor edits must compile live during the interaction.
...
| --------------------------------------- | ---------------------------------------------------------------- | ------------------------------------- | ------------------------------------------------ | ------------------------------------- | ----------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| `mouse-drag-outline`                    | `OutlineEditor` drag of nodes and sidebearings                   | `outline`                             | Yes, via `autoCompileManager.checkAndSchedule()` | Yes, `scheduleFullCompileDebounce()`  | `outline-only`    | `skip_features: true`, `skip_kerning: true`, `produce_varc_table: false` | Incremental sentinel JSON plus `update_cached_layer()`; cached subset key reused                      | Swap font blob and repaint only; skip HarfBuzz reshape  |
...
| `mouse-drag-anchor`                     | `OutlineEditor` anchor drag                                      | `anchor`                              | Yes                                              | Yes                                   | `anchor-only`     | `skip_kerning: true`, `produce_varc_table: false`                        | Incremental sentinel JSON plus `update_cached_layer()`; cached subset key reused                      | Swap font blob, reshape text, repaint                   |
...
| `mouse-drag-guide`                      | `OutlineEditor` guide drag                                       | `null`                                | Yes                                              | No                                    | `full`            | None                                                                     | Incremental sentinel JSON plus `update_cached_layer()` because it is still an interactive layer save  | Full compile/render path                                |
...
2. It wakes `autoCompileManager` immediately so the live fast path actually runs.
...
Removing step 2 breaks live feedback even if the optimized compile modes still exist.
...
1. `mouse-drag-outline` and `keyboard-outline` wake live compilation immediately and still schedule the trailing full compile.
2. `mouse-drag-anchor` and `keyboard-anchor` wake live compilation immediately and still schedule the trailing full compile.
