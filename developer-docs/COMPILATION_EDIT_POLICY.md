...
27. **Editing compile context is request-scoped and packet-explicit.** `lastChangeSource` / `lastEditType` are transient scheduling inputs only; committed local, remote, undo, and redo compiles MUST derive `{ changeSource, editType }` from the currently processed committed packet and pass that context explicitly when incrementing `compileRequestVersion`. `compileEditingFont()` MUST read the snapshot for the exact request it sends to Rust and MUST NOT fall back to ambient `lastChangeSource` / `lastEditType` when that request has no snapshot. Live drag refreshes may set the global context only long enough to request their compile, and SHOULD also pass the same explicit context into the request, then MUST clear the global state again so a late live request cannot poison the next committed, keyboard, undo, or redo compile. Interactive `saveLayerData()` may increment `compileRequestVersion` to invalidate stale in-flight live compiles, but it MUST NOT set `needsRecompile` or global compile context before the authoritative Yjs commit reaches the shared committed-change funnel. When `pendingBabelfontJsonSyncAfterDrag` is set, only an active `mouse-drag-*` live compile may continue on the incremental dirty-layer path without first resyncing canonical JSON; the next committed keyboard, undo, redo, remote, or deferred compile MUST resync from the current model before compiling so it does not build on a stale pre-drag base.
...
| --------------------------------------- | -------------------------------------------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mouse-drag-outline`                    | `OutlineEditor` drag of nodes and geometry transforms                | `outline`                             | Yes, via `refreshGlyphsAfterModelBatch()` + `autoCompileManager.checkAndSchedule()`  | Yes, final mouseup `saveLayerData()` arms the trailing full compile                                       | `outline-only`                                      | `skip_features: true`, `skip_kerning: true`, `produce_varc_table: false`       | Drag-time model sync plus incremental `refreshGlyphsAfterModelBatch()` for the edited layer and visible metrics dependents; final mouseup save restores the pending full-sync baseline                                                                                                                                                                                                                                        | Live editing compile during drag; trailing full compile waits for mouseup                                                                         |
| `mouse-drag-sidebearing`                | `OutlineEditor` sidebearing handle drag                              | `null` on the committed packet        | Yes, via visible-scoped recomposition loop + `autoCompileManager.checkAndSchedule()` | Yes, `scheduleFullCompileDebounce()` re-arms until drag end; mouseup calls `saveLayerData` for final sync | Live drag: `outline-only`; committed packet: `full` | None on the committed packet; live drag still uses outline-only worker options | No `saveLayerData` during drag; live refresh sends source + visible affected layers, then mouseup recomputes and syncs the complete affected layer batch                                                                                                                                                                                                                                                                      | Live drag keeps the outline-only path; the committed packet reshapes and repaints immediately after mouseup                                       |
...
3. **Builds compile context:** Derives `{ changeSource, editType }` from the committed packet being processed, not from ambient `lastChangeSource` / `lastEditType`.
...
| Compile context          | Set by edit handlers AND `saveLayerData` AND `requestCommittedEditingFontCompile` (chain of overwrites)                        | Request-scoped. The committed funnel passes an explicit context object derived from the current Yjs packet into the compile request; the live-drag funnel may use the global context only as a short-lived scratchpad and then clears it.             |
