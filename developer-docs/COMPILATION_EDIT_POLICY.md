...
22. **Node strings are the resting invariant; node arrays are runtime-only.** Persisted/resting path shapes MUST use upstream babelfont node strings in `.babelfont` JSON, cloud snapshots, Y.Doc room state, worker bootstrap snapshots, Rust-facing JSON, and compact Rust JSON caches. JavaScript editing/runtime code may decode those strings into `Babelfont.Node[]`, and Rust may deserialize them into native `Vec<Node>` caches, but those expanded representations MUST NOT leak back into resting storage. All JavaScript string/array conversion MUST go through `webapp/js/node-encoding.ts`; ad hoc parsers or serializers at render, Y.Doc, cloud, compile, or save call sites are forbidden. Editor-only node/path/component ids MUST NOT be emitted in saved or Y.Doc path/component storage. If malformed node data crosses a boundary, the central codec or Rust native-cache decoder MUST fail visibly instead of silently repairing it.
