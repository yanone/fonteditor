...
| Compile context | Set by edit handlers AND `saveLayerData` AND `requestCommittedEditingFontCompile` (chain of overwrites) | Set ONLY by the funnel, from inferred Yjs commit metadata. Edit handlers and drag live refresh no longer touch `lastChangeSource`/`lastEditType`. |
...
| `markDirty` | Called `fullCompileManager.checkAndSchedule()` as a side channel | Only marks font as changed. The full-compile manager's 200ms monitor interval detects the `changeVersion` increment naturally. |
...
| Dragging live refresh | Set compile context via `setEditingCompileContext` in refresh loops | No longer sets compile context — only the funnel manages `lastChangeSource`/`lastEditType`. |
