...
| `keyboard-outline`                      | `OutlineEditor` outline nudges and non-sidebearing structural saves  | `outline`                             | Yes, via sparse preview overlay while the burst is still idle-debounced              | Yes; the authoritative Yjs commit lands after the keyboard preview debounce, then the funnel arms the trailing full compile                              | `outline-only`                                      | `skip_features: true`, `skip_kerning: true`, `produce_varc_table: false`       | Arrow-key nudges mutate the local layer/model first, stage only sparse preview-layer overlay records against the authoritative worker subset, then commit one incremental Yjs packet after the idle debounce; no per-repeat worker-authoritative update is allowed                                                                                                                                                            | Preview compile swaps the editing blob live during the burst; the committed packet arrives once after idle and preserves one history/cloud step   |
| `keyboard-sidebearing`                  | `OutlineEditor` keyboard sidebearing adjustments (nudge, set)        | `null`                                | Yes; sidebearing nudges use the sparse preview overlay during the debounce window    | Yes; the committed `keyboard-sidebearing` packet lands once after the keyboard preview debounce, then the funnel may still arm its trailing full compile | `full`                                              | None                                                                           | Nudge bursts keep worker-authoritative state untouched until the debounced commit. Preview uses visible-scope sparse overlay refresh for the source layer plus dependent layers; the final packet still carries stamped `keyboard-sidebearing` compile metadata and mutates Rust only through the incremental Yjs update                                                                                                      | Live preview stays snappy on the drag-style fast path; the committed packet still reshapes and refreshes advances from one authoritative step     |
