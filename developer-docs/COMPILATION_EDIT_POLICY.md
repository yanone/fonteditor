...
15. Python edits MUST enter `PatchSyncEngine` and emit the same committed Yjs packet plus change-log metadata as every other persisted edit. Python may derive synthetic operations from normalized before/after font snapshots, including named path metadata, but it MUST first canonicalize the current font in place and refresh the serialized snapshot, then derive the post-execution snapshot from that freshly regenerated canonical serialization rather than from any stale pre-sync `babelfontJson` cache. It MUST NOT bypass the shared committed-change funnel with ad hoc full-font sync or direct post-commit reactions.
    Agent prompts open one `PatchSyncEngine` transaction before any prompt tool can mutate the font and close it exactly once when the prompt completes or is interrupted. Every bridge-recorded model mutation made during that interval, whether from Python synthetic operations or a future direct font-editing tool, belongs to that one transaction and therefore emits one authoritative incremental Yjs packet and one committed-funnel reaction. The history item uses the `set_prompt_history_summary` text when supplied, otherwise `Agent changes`; a Stop interruption appends ` (interrupted)`. Python snapshot diffs MUST preserve ordered collection fidelity through the Y.Doc schema: keyed glyph and layer maps use their dedicated order arrays, while ordinary JSON arrays use schema-compatible atomic replacements. This prompt grouping MUST NOT use `storeFontJson`, `seedYdoc`, `initYdoc`, or any full-document repair traffic.
...
30. **Agent shaping observes only settled committed worker state.** `compile_and_shape_font` MUST wait for both the worker-document sync and the current `FontManager.workerCacheUpdatePromise` before capturing its font revision or compiling. It MUST fail when the worker cache is not ready or an update rejects; it MUST NOT send repair traffic, full JSON, or a worker reseed. During active drag or keyboard preview state, as reported by `OutlineEditor.hasPendingKeyboardPreviewCommit()`, the tool MUST refuse both before and after its worker-state wait rather than silently compiling the older committed font, because its debug subset lane has no preview-overlay equivalent.
