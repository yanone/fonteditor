...
19. **Undo/redo replay-targets refresh keeps the worker cache congruent.** After `awaitLocalCommittedWorkerCacheSettled` resolves, the authoritative binary Yjs update has reached Rust through `forwardWorkerYjsUpdate`. For forward (non-undo/redo) local packets, `handleCommittedChangeRefresh` MUST NOT call `queueRustCacheAndRefreshCanvas` / `refreshWorkerCacheForReplayTargets` on the local sender path — the model data for all affected layers is current and the worker's Y.Doc-derived caches are already correct.

    For undo/redo packets, the JS-side font model (`currentFont.fontModel`) is NOT reverted for cascading recomposition layers: only the directly edited target glyph/layer is synced from the Y.Doc back to the JSON model via `_syncJsonFromYDoc`. Subsequent compile steps that use `forceFullEditingCacheRefresh` (enabled by undo/redo's `forceTrigger`) can re-serialize the stale model data and overwrite the worker's correct pre-edit caches. Therefore `handleCommittedChangeRefresh` MUST call `refreshWorkerCacheForReplayTargets` for undo/redo packets when replay targets exist beyond the direct entry paths. This extra refresh serializes all affected layers from the model — which, for undo/redo, are the pre-edit values the history item just restored — and pushes them as an incremental Yjs update to Rust. The receiver (remote) branch still runs the scoped cache refresh because linked windows need that pre-compile refresh on their own worker/cache side.

    See `webapp/js/change-bridge-init.ts` `handleCommittedChangeRefresh` and `awaitLocalCommittedWorkerCacheSettled`.
