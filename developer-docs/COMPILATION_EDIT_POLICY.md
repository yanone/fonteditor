...
| --------------------------------------- | -------------------------------------------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mouse-drag-outline`                    | `OutlineEditor` drag of nodes and geometry transforms                | `outline`                             | Yes, via `stageLiveDragPreviewFromModel()` + `autoCompileManager.checkAndSchedule()` | Yes, final mouseup `saveLayerData()` arms the trailing full compile                                       | `outline-only`                                      | `skip_features: true`, `skip_kerning: true`, `produce_varc_table: false`       | Drag-time model sync plus incremental `applyPreviewYjsUpdate` into transient worker preview state for the edited layer and visible metrics dependents; final mouseup commits the authoritative Yjs packet through the bridge/funnel                                                                                                                                                                                           | Live editing compile during drag from preview state; trailing full compile waits for mouseup                                                      |
| `mouse-drag-sidebearing`                | `OutlineEditor` sidebearing handle drag                              | `null` on the committed packet        | Yes, via visible-scoped recomposition loop + `autoCompileManager.checkAndSchedule()` | Yes, `scheduleFullCompileDebounce()` re-arms until drag end; mouseup calls `saveLayerData` for final sync | Live drag: `outline-only`; committed packet: `full` | None on the committed packet; live drag still uses outline-only worker options | No `saveLayerData` during drag; live refresh sends source + visible affected layers to transient worker preview state, then mouseup recomputes and syncs the complete affected layer batch through the authoritative Yjs packet                                                                                                                                                                                               | Live drag keeps the outline-only path; the committed packet reshapes and repaints immediately after mouseup                                       |
| `keyboard-outline`                      | `OutlineEditor` outline nudges and non-sidebearing structural saves  | `outline`                             | Yes                                                                                  | Yes                                                                                                       | `outline-only`                                      | `skip_features: true`, `skip_kerning: true`, `produce_varc_table: false`       | Incremental sentinel JSON plus `update_cached_layer()`; cached subset key reused                                                                                                                                                                                                                                                                                                                                              | Swap font blob and repaint only; skip HarfBuzz reshape                                                                                            |
| `keyboard-sidebearing`                  | `OutlineEditor` keyboard sidebearing adjustments (nudge, set)        | `null`                                | Yes                                                                                  | Yes                                                                                                       | `full`                                              | None                                                                           | Incremental batch via `refreshGlyphsAfterModelBatch`; cached subset key reused                                                                                                                                                                                                                                                                                                                                                | Full compile/render path so text advances and reshaping stay in sync                                                                              |
...
8. Live drag preview uses `applyPreviewYjsUpdate`, `primePreviewLayoutClosureCache`, and preview compile functions backed by transient Rust preview state cloned from the authoritative worker Y.Doc. Preview updates may compile immediately, but they MUST NOT mutate authoritative `Y_DOC`, `CANONICAL_JSON_CACHE`, `SUBSET_JSON_CACHE`, `FONT_CACHE`, `SUBSET_FONT_CACHE`, or the authoritative layout-closure sentinel. `clearPreviewYjsState` discards preview state; mouseup, undo, redo, remote changes, and keyboard edits converge only through `applyYjsUpdate` packets emitted or forwarded by the bridge.
...
1. Live dragging may update transient worker preview state and trigger instant editing recompilation before the final patch-funnel commit lands. It must not update authoritative worker state before mouseup. The drag interaction must still commit through the patch system when the gesture is finalized.
...
3. `keyboard-outline`, `keyboard-sidebearing`, and `keyboard-anchor` continue to wake compilation immediately through the Yjs commit → funnel path. The trailing deferred full compile is armed by the funnel after the edit is processed.
