...
19. **Local post-commit cache refresh skip for already-forwarded replay packets.** After `awaitLocalCommittedWorkerCacheSettled` resolves, `handleCommittedChangeRefresh` MUST skip the `queueRustCacheAndRefreshCanvas` / `refreshWorkerCacheForReplayTargets` call for local GUI commits whose Yjs worker update was already forwarded by `setYjsWorkerCallback`. This includes layer-scoped visual packets where every entry path matches `.layers.` or `:layers.` and carries non-empty `workerReplayTargets`, and master-reinterpolation batch packets labeled `Reinterpolate layer batch sync` that carry explicit layer `workerReplayTargets` while applying full glyph snapshots. The authoritative binary Yjs update already reached Rust through `forwardWorkerYjsUpdate`; a second replay-target cache refresh would duplicate serialization and a second `applyYjsUpdate`. The receiver (remote) branch still runs the cache refresh because linked windows need scoped layer refresh before their editing-font compile. See `webapp/js/change-bridge-init.ts` `handleCommittedChangeRefresh`.
...
3. **Sets compile context:** Calls `fm.setEditingCompileContext(changeSource, editType)` so the editing compile picks up the correct fast-path mode.
4. **Requests compile:** Calls `requestRecompileWithoutDataChange()` + `autoCompileManager.checkAndSchedule()`.
...
7. **Arms deferred full compile:** Calls `armDeferredFullCompile()` which replaces `FontManager.scheduleFullCompileDebounce()`.
...
| Compile context          | Set by edit handlers AND `saveLayerData` AND `requestCommittedEditingFontCompile` (chain of overwrites)                        | Request-scoped. The committed funnel sets context from inferred Yjs commit metadata; the live-drag funnel may set context only for the duration of its own compile request and then clears the global state.                                          |
...
The deferred full compile timer lives inside the funnel (`armDeferredFullCompile`). It is armed after every `processCommittedEdit` call for fast-path edit types. The timer:
...
10. Undo/redo of sidebearing edits uses `outline-only` compilation mode and the already-forwarded sparse Yjs update when the history item carries `workerReplayTargets` for downstream layers affected by metrics-key cascades.
