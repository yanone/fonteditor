...
That script starts the Preview Release workflow. The workflow waits for a green CI run on that commit, then builds with `preview-build-N-on-DATE` and creates the `v0.0.0-preview-DATE-SHA` tag and prerelease. Notes are the Unreleased changelog diff since the previous preview. It does not rewrite `CHANGELOG.md` or re-run the test suite.
