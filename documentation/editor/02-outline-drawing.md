...
**Subtract** in the property panel marks selected paths as cutters (`fip001-boolean`). They punch a hole in everything below them in shape order; paths drawn later sit on top. Nodes stay editable. Cutter strokes are dashed, and the cutter fill uses the same transparency as the path fill, minus the overlap with additive paths so the hole matches the compiled ink. Glyph overview tiles punch the same holes. **Fill** in Editing View → View sets that shared opacity. Compile flattens any glyph that uses a cutter (including through components) and runs the same boolean. Linked layers get the flag at the same shape indexes.
