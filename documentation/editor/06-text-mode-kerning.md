...
You can also open **Font → Kerning Editor…** (`Cmd/Ctrl + Shift + K`) for a table of all defined pairs (LTR or RTL) with values per master. From text mode, that shortcut selects the active First/Second chips from the property panel (glyph or class) and scrolls that row into view. Select a row to delete it from every master, or edit individual cells. Undo restores the previous values and the table UI state.
