...
**Font → Convert to Counterpunch** looks for glyphs where Counterpunch's composition engine can place every component — by the current anchor rules, or as a single non-mark component — and where that placement matches the stored positions **and layer width** on every layer of the glyph. Only then does it mark those components automatic. If any layer would move, if the derived advance would differ from the stored width (typical for tabular figures whose slot was widened by hand), or if a component has no matching anchors for the engine (and is not that single non-mark case), the glyph stays manual.
