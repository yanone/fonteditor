...
Search by name (`Cmd/Ctrl+F` focuses the search field), change tile size to balance scanning and detail, and select one or more glyphs. Double-click a glyph tile to edit that glyph in the Editor. `Cmd/Ctrl+D` duplicates the glyph selection. `Cmd/Ctrl+Shift+F` renames it. Delete or Backspace deletes it after confirm.
