...
```python
font = Font()      # open font; raises if none is open
glyph = Glyph()    # glyph being edited in outline mode
layer = Layer()    # layer being edited in outline mode
master = Master()  # selected master, or None off a stored master
```
...
The object model is documented in [Python API](06-python-api.md). Start writing in [Script editor](02-script-editor-workflow.md). Quick commands are in [Konsole](03-konsole-quick-tasks.md).
