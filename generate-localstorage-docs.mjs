...
function getLocalStorageMethod(node) {
    ...
    if (ts.isIdentifier(target) && target.text === "localStorage") {
        return method;
    }
    if (
        ...
        ((ts.isIdentifier(target.expression) &&
            ...
            target.name.text === "localStorage") ||
            (ts.isIdentifier(target.expression) &&
                target.expression.text === "js" &&
                target.name.text === "localStorage"))
    ) {
        return method;
    }
...
function collectTypeScriptKeys(filePath) {
    ...
    function visit(node) {
        const method = getLocalStorageMethod(node);
        if (method) {
...
function collectPythonKeys(filePath) {
...
function main() {
    ...
    const pyFiles = walkFiles(PLUGIN_SOURCE_ROOT, new Set([".py"]));

    const usages = [
        ...tsFiles.flatMap(collectTypeScriptKeys),
        ...pyFiles.flatMap(collectPythonKeys),
    ];
    ...
    if (unresolved.length) {
        console.error(
            `Unresolved localStorage key expression(s):`,
        );
    ...
    writeFileSync(OUTPUT_PATH, renderMarkdown(keys), "utf-8");
    console.log(
        `Generated ${keys.length} localStorage key reference entr${keys.length === 1 ? "y" : "ies"} at ${formatRelativePath(OUTPUT_PATH)}`,
    );
}
