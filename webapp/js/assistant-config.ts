...
export const ASSISTANT_TOOLS: AssistantTool[] = [
    ...
    },
    {
        ...
        function: {
            ...
            description:
                'Return the central Glyph Overview filter event list plus a compact single-file example. You must call this before authoring or substantially editing a glyph filter so its required EVENT_TYPES and optional apply_changes(change_batch, current_results, font) declarations use supported event types. Subscribe EVENT_TYPES only to events the filter must handle for correctness; do not over-subscribe to unrelated types.',
            parameters: {
                type: 'object',
                properties: {},
                required: []
    ...
    },
    {
        ...
        function: {
            ...
            description:
                'After create_python_draft_in_editor or replace_python_text_in_editor, perform non-executing validation of the current Script Editor buffer. This parses Python syntax with the Python compiler but does not execute the code object. The result includes kind, editorKind, kindConfidence, and kindMessage because unsaved pathless buffers can be unclassified even when the editor fallback is general-script. Glyph filters must define EVENT_TYPES and filter_glyphs(font); apply_changes(change_batch, current_results, font) is optional. Report validation failures or unclear kind classification before proposing more edits. This never runs user code, imports the user document, or saves Python.',
            parameters: {
                type: 'object',
                properties: {},
                required: []
