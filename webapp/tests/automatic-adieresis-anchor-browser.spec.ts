...
async function installEditingFontCompileTracker(page: Page): Promise<void> {
    await page.evaluate(() => {
        ...
        if (testWindow.__automaticAdieresisCompileTrackerInstalled) {
            return;
        }

        testWindow.__automaticAdieresisCompileCount = 0;
        window.addEventListener('editingFontCompiled', () => {
            testWindow.__automaticAdieresisCompileCount += 1;
        });
        testWindow.__automaticAdieresisCompileTrackerInstalled = true;
    });
}

async function getEditingFontCompileCount(page: Page): Promise<number> {
    return page.evaluate(
        () => (window as any).__automaticAdieresisCompileCount ?? 0
    );
}

async function waitForNextEditingFontCompile(
    page: Page,
    previousCount: number
): Promise<void> {
    await page.waitForFunction(
        (count) =>
            ((window as any).__automaticAdieresisCompileCount ?? 0) > count,
        previousCount,
        { timeout: 30000 }
    );
}
...
test.describe('automatic adieresis anchor browser commit', () => {
    test('browser font/model state follows the committed adieresis layer after dragging a.top', async ({
        ...
        const beforeCompileCount = await getEditingFontCompileCount(page);
        const expectedTranslation: [number, number] = [70, 797];
        await commitExplicitAdieresisLayerSetThroughBridge(
            page,
            expectedTranslation
        );
        ...
        await page.waitForFunction(
            ({ nextTranslation }) => {
                ...
                const entries = win.patchSyncEngine?.getChangeLog?.() || [];
                const recentLayerEntry = entries
                    .slice()
                    .reverse()
                    .find(
                        (entry: any) =>
                            (entry?.path ===
                                `glyphs.adieresis:layers.${layerId}:` ||
                                entry?.path ===
                                    `glyphs.adieresis:layers.${layerId}`) &&
                            entry?.newValue?.shapes
                    );
                const mark = recentLayerEntry?.newValue?.shapes?.find(
                    (shape: any) => shape?.reference === 'dieresiscomb'
                );
                const fullLayerTranslation = mark?.transform?.translation;
                if (
                    Array.isArray(fullLayerTranslation) &&
                    fullLayerTranslation[0] === nextTranslation[0] &&
                    fullLayerTranslation[1] === nextTranslation[1]
                ) {
                    return true;
                }
                const xEntry = entries
                    .slice()
                    .reverse()
                    .find(
                        (entry: any) =>
                ...
                return (
                    xEntry?.newValue === nextTranslation[0] &&
                    yEntry?.newValue === nextTranslation[1]
                );
            },
            { nextTranslation: expectedTranslation },
            { timeout: 15000 }
        );
        ...
        await waitForNextEditingFontCompile(page, beforeCompileCount);

        const postCompileState = await getAdieresisCommitState(page);
        expect(
            postCompileState.workerYDocTranslation,
            JSON.stringify(postCompileState)
        ).toEqual(expectedTranslation);
        expect(
            postCompileState.workerCanonicalTranslation,
            JSON.stringify(postCompileState)
        ).toEqual(expectedTranslation);
        if (postCompileState.workerSubsetTranslation !== null) {
            expect(
                postCompileState.workerSubsetTranslation,
                JSON.stringify(postCompileState)
            ).toEqual(expectedTranslation);
        }
    });
